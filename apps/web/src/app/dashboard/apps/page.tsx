...
import { Boxes, Plus, Loader2, Copy, Check, AlertTriangle } from "lucide-react"
...
export default function AppsPage() {
  ...
  async function create() {
    ...
    try {
      ...
      setCreated(res)
      setShowCreate(false)
      setName(""); setRedirects(""); setLogouts(""); setIsPublic(false)
      await load()
    } catch (err) {
      toast({ title: "Error", description: formatApiError(err instanceof Error ? err.message : undefined), variant: "destructive" })
    } finally {
    ...
    <>
      ...
      <div className="w-full max-w-4xl px-4 py-6 sm:px-6 space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button size="sm" onClick={() => setShowCreate(true)}><Plus className="w-3.5 h-3.5" /> Registrar app</Button>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-lg border p-12 text-center">
            ...
            <Button size="sm" onClick={() => setShowCreate(true)}><Plus className="w-3.5 h-3.5" /> Registrar la primera</Button>
          </div>
        ) : (
            ...
            {clients.map((c) => (
              <div key={c.id} className="rounded-lg border p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <code className="font-mono text-xs text-muted-foreground">{c.clientId}</code>
                </div>
